...
이 문서는 `notion-note-sync`의 설계 선택과 제한 사항을 정리한다. 이 프로젝트는 작은 개인 자동화 도구이며, 범위를 넓히기보다 권한과 실행 흐름을 명확히 두는 데 초점을 둔다.

## 문제 정의

Notion은 웹 편집 경험은 좋지만, 반복적으로 작성하는 노트나 학습 기록을 매번 웹에서 수정하기에는 번거롭다. 이 프로젝트는 그 반복 작업을 줄이기 위해 로컬 Markdown 파일을 기준으로 Notion 페이지를 갱신한다.
...
이 프로젝트는 Notion API를 직접 호출한다. 동기화 대상은 사용자가 `config/pages.json`에 적은 Page ID와 Markdown 파일로 제한한다.

이유:

* **권한 범위가 명확하다.** Notion Integration이 연결된 페이지에만 접근한다.
* **실행 흐름이 고정되어 있다.** Markdown 읽기, 백업, Notion 교체, 결과 출력 순서가 코드로 정해져 있다.
* **쓰기 동작이 사용자 명령에 묶여 있다.** 사용자가 명령을 실행할 때만 Notion 수정이 발생한다.
...
* Notion의 모든 블록 타입을 세밀하게 편집하는 도구는 아니다.

## 이 프로젝트에서 확인할 수 있는 부분

작은 도구지만 다음 부분은 명확히 다루려고 했다.
